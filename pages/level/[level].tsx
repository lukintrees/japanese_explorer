...
function Level(props){
    ...
    return <><NavBar/><div className="container mx-5 md:mx-auto bg-white dark:bg-black pt-20 px-10 dark:text-white">
                <h1 className="text-3xl font-sans my-1">{"Level " + router.query['level']}</h1>
...
export const getStaticPaths: GetStaticPaths = async () => {
    var paths = []
    for(var i = 1;i<61;i++){
        paths.push({params:{level:i.toString()}})
    }
    return {paths,fallback:false}
}

export const getStaticProps: GetStaticProps = async (context) => {
    const radicals = await getRadicalsByLevel(parseInt(context.params.level.toString()))
    const kanji = await getKanjiByLevel(parseInt(context.params.level.toString()))
    const vocabulary = await getVocabularyByLevel(parseInt(context.params.level.toString()))
