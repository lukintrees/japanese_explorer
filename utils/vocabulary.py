...
def getContextSent(temp):
    final = []
    for i in temp:
        final.append(i[0].text.replace('"',"'")+"&"+i[1].text.replace('"',"'"))
    return final

def getVocabulary(levels):
    vocabularys = []
    ...
    for vocabulary in vocabularys:
        ...
        write(id1,level,jpname,name,altname,wordtype,"`".join(mnemonic),json.dumps(reading).replace('"',"'"),"`".join(readmnemonic).replace('"','"'),context,contextsent,",".join(kanjis))
        bar.next()
    bar.finish()

url = "http://www.wanikani.com"
getVocabulary(input("Levels: "))